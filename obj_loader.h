...
inline face read_face(std::vector<std::string>& tokens) {
    ...
    for (int i = 1; i < tokens.size(); i++) {
        ...
        if (indices.size() >= 2 && !indices[1].empty()) texture_indices.push_back(std::stoi(indices[1]));
        if (indices.size() == 3) normal_indices.push_back(std::stoi(indices[2]));
...
std::shared_ptr<triangle_mesh> get_triangle_mesh_from_file(std::string filename) {
